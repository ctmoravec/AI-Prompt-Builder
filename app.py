...
def set_theme():
    ...
    .clear-tab button:hover{ color:var(--text) !important; border-bottom-color:var(--accent) !important; }
    </style>
        ...
        with st.expander("Create New Element", expanded=False):
            ...
            if st.button("Add Element", key="add_element"):
                if not title: st.error("Title is required.")
                    ...
                    newrow=pd.DataFrame({'title':[title],'type':[element_type],'content':[content]})
...
# =========================
# Prompt Browser & Backup/Restore (unchanged)
# =========================
class PromptBrowser:
    @staticmethod
    def render():
...
def render_backup_restore_tab():
    ...
    with c2:
        ...
        if up2 is not None:
            try:
                new=pd.read_csv(up2); base=DataManager.load_data('prompt_history.csv', PROMPT_HISTORY_COLUMNS)
                comb=pd.concat([base,new],ignore_index=True).drop_duplicates(subset=PROMPT_HISTORY_COLUMNS, keep="last")
...
CLEAR_KEYS = [
    ...
    # custom text fields tied to "Write your own"
...
def clear_form_state():
    ...
    # no st.rerun() needed—Streamlit will rerun after the button click automatically
...
def main():
    ...
    # Row with 5 tabs + a faux-tab Clear button on the same line
    tabs = st.tabs(["Element Creator","Element Editor","Prompt Builder","Browse Prompts","Backup / Restore"])
    # Render the faux-tab button right below the tab list so it appears inline
    st.markdown('<div style="height:0.25rem"></div>', unsafe_allow_html=True)
    clear_col = st.container()
    with clear_col:
        st.button("Clear Form", key="clear_form_btn", on_click=clear_form_state, help="Clear all dropdowns and the prompt box", type="secondary", disabled=False, use_container_width=False)
    st.markdown('<style>.clear-tab{display:inline-block;margin-left:8px}</style>', unsafe_allow_html=True)
